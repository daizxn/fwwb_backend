...
class BertForTokenClassification(BertPreTrainedModel):
    ...
    def __init__(self, config, label_smoothing=0.0):
        super().__init__(config, label_smoothing=0.0)
        self.num_labels = config.num_labels

        self.bert = BertModel(config, add_pooling_layer=False)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.classifier = self.build_mlp(input_dim=config.hidden_size, output_dim=config.num_labels)
    ...
    def forward(
        ...
        sequence_output = self.dropout(sequence_output[:,1:]) # [:,1:] for ingoring class token
        ...
        if labels is not None:
            ...
            if attention_mask is not None:
                attention_mask_rm_CLS = attention_mask[:,1:] # [:,1:] for ingoring class token
                active_loss = attention_mask_rm_CLS.reshape(-1) == 1
                active_logits = logits.view(-1, self.num_labels)
        ...
        if soft_labels is not None:
            ...
            loss_distill = loss_distill[labels_reshape!=-100].mean() # -100 index = padding token
